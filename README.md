...
- **制片人审阅** —— 审核外部文件夹中批量生成的创作素材，标记通过/驳回并记录修改意见
...
制片人审阅页用于审核**外部文件夹**中 AI 批量生成的创作素材（如分镜关键帧、角色设定图、场景图），逐条标记审核结论并记录修改意见，适合"生成 → 审阅 → 返工"的生产闭环。

**配置外部素材目录**

素材目录通过 `.env` 挂载进容器（只读，不会写入或修改你的原始文件）：

```bash
# 宿主机素材目录绝对路径（请使用正斜杠）
REVIEW_SOURCES_HOST_PATH=C:/Users/your-name/Documents/项目素材库
# 容器内允许扫描的根目录（与上面的挂载点对应，逗号分隔可配多个）
REVIEW_SOURCE_ROOTS=/app/external_sources
```

未配置时回退到 `./data/review_sources`，可先把素材放进该目录快速试用。

> 出于安全考虑，审阅只允许扫描 `REVIEW_SOURCE_ROOTS` 指定目录及其子目录下的文件，其他路径一律拒绝。

**核心能力：**

- **递归扫描** —— 自动递归识别目录下的图片/视频/音频文件，保留子目录层级（如 `01_characters/zeus/ZEUS_PRIMARY.png`）
- **缩略图预览** —— 扫描时为图片与视频生成缩略图（存于 `./data/review_thumbs/`，不写入你的素材目录），点击可放大查看原图
- **四态标记** —— 待审 / 通过 / 需修改 / 驳回，支持按状态筛选与"全部通过待审"批量操作
- **修改意见** —— 每条素材可编辑文字修改意见，随时保存，供后续返工参考
- **重新扫描** —— 素材目录有新产物时一键同步，新增文件自动入库、已删除文件自动移除

**简单使用：**

1. 在侧边栏点击「审阅」进入审阅页；
2. 点击「新建审阅」，填写会话标题并选择要审核的素材文件夹；
3. 系统自动扫描文件夹中的素材并生成缩略图，逐条查看并标记状态、填写修改意见；
4. 素材有更新时点击「重新扫描」同步，全部审完后点击「标记完成」归档该轮审阅。

> 删除审阅会话只会移除审阅记录与缩略图，**不会删除外部素材文件夹中的原始文件**。
